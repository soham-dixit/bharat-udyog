...
const ProductsAll = () => {
  ...
  const handleDeleteProduct = async (productId) => {
    ...
    try {
      const res = await axios.delete(`/exporter/deleteProduct/${productId}`);
      toast.success(res.data.message);
  
      setData((prevData) => prevData.filter((product) => product._id !== selectedDeleteProduct._id));
  ...
  const fetchData = async () => {
    await axios
      .get(`/exporter/getAllProducts/${state.user.id}`)
      .then((res) => {
        toast.success(res.data.message);
        setData(
          res.data.products.slice(
  ...
  async function openModal(productId) {
    let product = data.filter((product) => product._id === productId)[0];
    // console.log(product);
    ...
    <div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <Label className="mr-8">
                ...
                <div className="relative text-gray-500 focus-within:text-color dark:focus-within:text-red-400">
                  <input
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal}>
        ...
        <ModalFooter>
          ...
          <div className="hidden sm:block">
            <Button onClick={() => handleDeleteProduct(selectedDeleteProduct._id)}>Delete</Button>
          </div>
                  ...
                  <TableRow key={i}>
                    ...
                    <TableCell>
                      <div className="flex">
                        ...
                        <Link to={`/app/editproduct/${product._id}`}>
                          <Button
                            icon={EditIcon}
                            className="mr-3"
                            layout="outline"
              ...
              <div className="" key={product.id}>
                <Card>
                  <img
                    ...
                    // src={`https://dak-ghar-niryat-kendra.vercel.app/images/${product.photoUrl}`}
                    src={product?.photoUrl}
                    alt="product"
                  />
                  <CardBody>
                    <div className="mb-3 flex items-center justify-between">
                      <p className="font-semibold truncate  text-grlay-600 dark:text-gray-300">
                        {product.productName}
                      </p>
                      <Badge
