...
const OrdersTable = ({ resultsPerPage, filter ,dateSort}) => {
  ...
  useEffect(() => {
    ...
    // If Filters Applied
    if (filter === "PostOffice approved") {
      filteredData = response.filter((order) => order.status === "PostOffice approved");
    } else if (filter === "Custom approved") {
      filteredData = response.filter((order) => order.status === "Custom approved");
    } else if (filter === "Custom detained") {
      filteredData = response.filter((order) => order.status === "Custom detained");
