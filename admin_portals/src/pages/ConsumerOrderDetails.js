...
const ConsumerOrderDetails = () => {
  ...
  const shipToCustom = async () => {};

  const verifyOrder = async () => {
    toast.loading("Verifying order");
    await axios
      .put(`/postmaster/verifyOrder/${id}`)
      .then((res) => {
        toast.remove();
  ...
  const fetchPostman = async () => {
    await axios
      .get(`/postmaster/getPostman/${state.user.id}`)
      .then((res) => {
        toast.success(res.data.message);
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              {status == "Under Post Office Verification" && (
                <>
                  <Button className="mt-3" onClick={verifyOrder}>
                    Verify Order
                  </Button>
                </>
              )}
              {status == "Verified by Post Office" && (
                <>
                  <Button className="mt-3" onClick={shipToCustom}>
                    Ship to custom
                  </Button>
                </>
              )}
